...
# Shared models from osm2pgsql-query-builder
...
def _validate_geometry_or_iso3(geometry, iso3):
    if geometry is not None and iso3 is not None:
        raise ValueError("Only one of geometry or iso3 should be supplied.")
    if geometry is None and iso3 is None:
...
class RawDataCurrentParamsBase(SnapshotQueryParams):
...
class DynamicCategoriesModel(CategoriesBase, GeometryValidatorMixin):
    ...
    @classmethod
    def set_geometry_or_iso3(cls, value, info: ValidationInfo):
        ...
        if value is not None:
            ...
            if info.data.get("hdx_upload"):
                for category in info.data.get("categories"):
                    _, category_data = next(iter(category.items()))
