...
def parse_arguments():
    ...
    parser.add_argument(
        ...
        choices=["d", "w", "m", "h"],
        default="d",
        help="H3 field update frequency",
    )
    parser.add_argument(
        "--no-replication",
        dest="replication",
        action="store_false",
        help="Disable replication (replication is ON by default)",
    )
    parser.set_defaults(replication=True)
    parser.add_argument(
        "--flat_nodes", type=str, help="Flat-nodes option for osm2pgsql"
    )
    parser.add_argument("--cache", type=str, help="Cache size for osm2pgsql")
    parser.add_argument(
        "--boundary", type=str, help="Boundary geojson for replication filtering"
    )
    parser.add_argument(
        "--skip_h3update", action="store_true", help="Skip H3 spatial index update"
    )
    parser.add_argument(
        "--insert", action="store_true", help="Run osm2pgsql to insert data"
    )
    parser.add_argument(
        "--update", action="store_true", help="Run H3 spatial index update on tables"
    )
    parser.add_argument(
        "--download_dir", type=str, help="Directory to download source file"
    )
    parser.add_argument(
        "--post_index", default=False, action="store_true", help="Run post index only"
    )
    parser.add_argument(
        "extra_params",
        nargs="*",
        metavar="param",
        help="Extra params to pass to osm2pgsql",
    )
...
def setup_database_prerequisites(dbname, host, port, user):
    ...
    missing = [
        ext
        for ext, _ in required_extensions
        if run_psql_query(
            f"SELECT 1 FROM pg_available_extensions WHERE name='{ext}'",
            "postgres",
            host,
            port,
            user,
...
def main():
    ...
    if len(target_paths) > 1:
        ...
        if not os.path.exists(merged_path):
            ...
            subprocess.run(
                ["osmium", "merge", *target_paths, "-o", merged_path], check=True
            )
