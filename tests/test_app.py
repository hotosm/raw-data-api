...
def test_rawdata_current_snapshot_geometry_query():
    ...
    expected_query = """select ST_AsGeoJSON(t0.*) from (select
            ...
            from
                relations
            where
                ST_intersects(geom,ST_GEOMFROMGEOJSON('{"type": "Polygon", "coordinates": [[[84.92431640625, 27.766190642387496], [85.31982421875, 27.766190642387496], [85.31982421875, 28.02592458049937], [84.92431640625, 28.02592458049937], [84.92431640625, 27.766190642387496]]]}'))) t3"""
...
def test_rawdata_current_snapshot_normal_query_ST_within():
    ...
    expected_query = """select ST_AsGeoJSON(t0.*) from (select
            ...
            from
                relations
            where
                ST_within(geom,ST_GEOMFROMGEOJSON('{"type": "Polygon", "coordinates": [[[84.92431640625, 27.766190642387496], [85.31982421875, 27.766190642387496], [85.31982421875, 28.02592458049937], [84.92431640625, 28.02592458049937], [84.92431640625, 27.766190642387496]]]}'))) t3"""
    query_result = raw_currentdata_extraction_query(
        validated_params,
    )
    assert query_result.encode("utf-8") == expected_query.encode("utf-8")
...
def test_attribute_filter_rawdata():
    ...
    expected_query = """select ST_AsGeoJSON(t0.*) from (select
            ...
            from
                relations
            where
                ST_intersects(geom,ST_GEOMFROMGEOJSON('{"type": "Polygon", "coordinates": [[[83.502574, 27.569073], [83.502574, 28.332758], [85.556417, 28.332758], [85.556417, 27.569073], [83.502574, 27.569073]]]}')) and (tags ->> 'building' = 'yes') and (geometrytype(geom)='POLYGON' or geometrytype(geom)='MULTIPOLYGON')) t3"""
    query_result = raw_currentdata_extraction_query(
        validated_params,
        g_id=[[1187], [1188]],
    )
