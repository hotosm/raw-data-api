...
def test_rawdata_current_snapshot_geometry_query() :
    ...
    expected_query = """select ST_AsGeoJSON(t0.*) from (select
            ...
            from
                relations
            where
                ST_intersects(ST_GEOMFROMGEOJSON('{"coordinates": [[[84.92431640625, 27.766190642387496], [85.31982421875, 27.766190642387496], [85.31982421875, 28.02592458049937], [84.92431640625, 28.02592458049937], [84.92431640625, 27.766190642387496]]], "type": "Polygon"}'), geom) and (geometrytype(geom)='MULTILINESTRING')) t2 UNION ALL select ST_AsGeoJSON(t3.*) from (select
            osm_id ,tags::text as tags,changeset,timestamp::text,geom
            from
                ways_poly
            where
                ST_intersects(ST_GEOMFROMGEOJSON('{"coordinates": [[[84.92431640625, 27.766190642387496], [85.31982421875, 27.766190642387496], [85.31982421875, 28.02592458049937], [84.92431640625, 28.02592458049937], [84.92431640625, 27.766190642387496]]], "type": "Polygon"}'), geom)) t3 UNION ALL select ST_AsGeoJSON(t4.*) from (select
            osm_id ,tags::text as tags,changeset,timestamp::text,geom
            from
                relations
            where
                ST_intersects(ST_GEOMFROMGEOJSON('{"coordinates": [[[84.92431640625, 27.766190642387496], [85.31982421875, 27.766190642387496], [85.31982421875, 28.02592458049937], [84.92431640625, 28.02592458049937], [84.92431640625, 27.766190642387496]]], "type": "Polygon"}'), geom) and (geometrytype(geom)='POLYGON' or geometrytype(geom)='MULTIPOLYGON')) t4"""
